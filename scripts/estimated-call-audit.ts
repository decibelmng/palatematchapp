/**
 * Would barring estimated lines from the Call change the Call?
 *
...
let listScans = 0, changed = 0, noCatalog = 0, sameBottle = 0;
...
for (const [scanId, ls] of scans) {
  ...
  for (const l of ls) {
    ...
    meta.set(key, { ...l, isCatalog, type: t });
  }
  ...
  const now = pick(solid(ranked));
  const catalogOnly = solid(ranked).filter((r) => meta.get(r.bottle.id)!.isCatalog);
  if (catalogOnly.length === 0) noCatalog++;
  const next = pick(catalogOnly.length > 0 ? catalogOnly : solid(ranked));
  if (!now || !next) continue;
  if (now.bottle.id === next.bottle.id) { sameBottle++; continue; }
  changed++;
  details.push(
    `${scanId.slice(0, 8)}  now: ${now.bottle.name} (${meta.get(now.bottle.id)!.isCatalog ? "catalog" : "estimated"}, ${now.predicted.toFixed(2)})  ->  ${next.bottle.name} (catalog, ${next.predicted.toFixed(2)})  Δ${(now.predicted - next.predicted).toFixed(2)}`,
  );
}

console.log(JSON.stringify({ listScans, unchanged: sameBottle, changed, scansWithNoCatalogMatch: noCatalog }, null, 2));
