...
import { recommend, selectBandwidth, type BottleFp, type RatedFp, type Recommendation, type WineType } from "@/lib/recommender";
    ...
    for (const type of TYPE_ORDER) {
      ...
      if (sameTypeRated.length === 0) {
        ...
        if (type === "red") {
          ...
          console.log(`[matches:bw] type=red bw=${bw} n=${preds.length} min=${Math.min(...preds).toFixed(4)} max=${Math.max(...preds).toFixed(4)}`);
        }
